...
export default function DangerousGoodsPage() {
  ...
  const cfg = AC[type];
  const Icon = cfg.icon;
  const [tab, setTab] = useState<'cases' | 'agents' | 'faq' | 'knowledge'>('agents');

  const tabs = [
    { key: 'cases' as const, label: '走货实例', icon: ClipboardList },
    { key: 'agents' as const, label: '危险品代理', icon: Building2 },
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">寻找危险品代理</h1>
          <p className="text-sm text-gray-500 mt-0.5">危险品代理 · 走货实例 · FAQ · 知识库</p>
        </div>
      </div>
