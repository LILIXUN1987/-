...
export default function InspectorDirectoryPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['inspectors', search, page],
    queryFn: () => directoryApi.inspectors({ search: search || undefined, page, limit: 12 }),
  });
  ...
  const handleConsultSend = async () => {
    ...
    try {
      await client.post(`/messages/legal-consult/${consultTarget.id}`, {
        content: `【检测认证咨询】${consultText.trim()}`,
      });
      setConsultSent(true);
      setConsultText('');
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            {lang === 'en' ? 'Inspection & Certification' : '检测认证黄页'}
          </h1>
          <p className="text-sm text-gray-500 mt-0.5">
            {lang === 'en' ? 'Find inspection and certification service providers' : '查找检测认证服务商，在线咨询与联系'}
          </p>
        </div>
      </div>

      {/* Notice */}
      <div className="bg-teal-50 border border-teal-200 rounded-xl px-5 py-4 mb-5">
        <div className="flex items-start gap-3">
          <Shield className="w-5 h-5 text-teal-600 mt-0.5 flex-shrink-0" />
          <div className="text-sm text-teal-800 space-y-1">
            <p className="font-medium">{lang === 'en' ? '📋 Notice' : '📋 说明'}</p>
            <p>{lang === 'en' ? 'Select a provider and click "Consult" to send an internal message.' : '选择您想咨询的服务商，点击「咨询TA」即可发送站内信。'}</p>
            <p className="text-xs text-teal-600">{lang === 'en' ? '* Consultation is for reference only' : '* 咨询内容仅供参考'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="relative mb-5">
        ...
        <input className="w-full pl-10 pr-16 py-2.5 text-sm border-2 border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-teal-400 focus:border-teal-400 placeholder-gray-400 bg-white transition-all"
          placeholder={lang === 'en' ? 'Search company or service...' : '搜索公司名称或服务类型...'}
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
          ...
          <p className="text-gray-400 text-sm font-medium">
            {search
              ...
              : (lang === 'en' ? 'No inspection providers yet' : '暂无检测认证服务商')}
          </p>
        ...
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {items.map((item) => {
              ...
              return (
                <div key={item.id} className="bg-white rounded-xl border border-gray-200 hover:border-teal-300 hover:shadow-md transition-all p-5">
                  <div className="flex items-start gap-4">
                    <div className="w-14 h-14 rounded-full flex-shrink-0 overflow-hidden bg-teal-100 flex items-center justify-center ring-2 ring-teal-200/50">
                      {avatarUrl ? (
                        <img src={avatarUrl} alt={item.display_name} className="w-full h-full object-cover"
                          onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; (e.target as HTMLImageElement).parentElement!.innerText = item.display_name?.charAt(0) || '检'; }} />
                      ) : (
                        <span className="text-xl font-bold text-teal-700 select-none">{item.display_name?.charAt(0) || '检'}</span>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2">
                        <button className="flex items-center gap-1.5 text-sm px-4 py-2 rounded-lg font-medium bg-teal-50 text-teal-700 hover:bg-teal-100 transition-colors border border-teal-200"
      ...
      {consultTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => { if (!consultSending) { setConsultTarget(null); setConsultSent(false); } }}>
          <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-md mx-4 border-t-4 border-teal-500" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-3">
              <h3 className="font-bold text-gray-900 text-base">🔬 咨询 {consultTarget.name}</h3>
            ...
            {consultSent ? (
              ...
              <>
                ...
                <p className="text-xs text-gray-500 mb-3">{lang === 'en' ? 'Describe your needs' : '请描述您的检测认证需求'}</p>
                <textarea className="input-field w-full min-h-[120px] text-sm resize-none mb-3"
                  placeholder={lang === 'en' ? 'Describe your inspection needs...' : '请输入您的检测认证需求...'}
                  value={consultText} onChange={e => setConsultText(e.target.value)} disabled={consultSending} autoFocus />
                <button className="w-full flex items-center justify-center gap-2 text-sm py-2.5 rounded-lg font-medium text-white bg-teal-600 hover:bg-teal-700 disabled:opacity-50 transition-colors"
